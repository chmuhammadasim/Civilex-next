...
export type WrittenStatementStatus = "draft" | "filed";

/** One per-allegation response entry stored in the JSONB column */
export interface SpecificResponse {
  allegation_number: number;
  allegation_summary: string;
  response: string;
  admission: "admitted" | "denied" | "not_known";
}

/** Defendant's formal reply to the plaintiff's plaint (Phase 3) */
export interface WrittenStatement {
  id: string;
  case_id: string;
  filed_by: string | null;
  filed_at: string | null;
  status: WrittenStatementStatus;
  general_denial: string | null;
  specific_responses: SpecificResponse[];
  preliminary_objections: string | null;
...
export interface WrittenStatementWithRelations extends WrittenStatement {
  filer?: { id: string; full_name: string } | null;
}
...
export type AttendanceSide = "plaintiff" | "defendant" | "court" | "other";

/** Per-hearing roster entry — who was present at a sitting (Phase 5) */
...
export const ATTENDANCE_ROLE_LABELS: Record<AttendanceRole, string> = {
  ...
  plaintiff: "Plaintiff",
  defendant: "Defendant",
  plaintiff_lawyer: "Plaintiff's Lawyer",
  defendant_lawyer: "Defendant's Lawyer",
  stenographer: "Stenographer / Reader",
  witness: "Witness",
  other: "Other",
};

export const ATTENDANCE_SIDE_LABELS: Record<AttendanceSide, string> = {
