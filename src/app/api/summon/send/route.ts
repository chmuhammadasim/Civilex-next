...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!profile || !["admin_court", "trial_judge"].includes(profile.role)) {
...
`.trim();
    ...
    // Send email to defendant if they provided an email address
    let emailSent = false;
    if (caseRow.defendant_email) {
      emailSent = await sendCaseEmail({
        to: caseRow.defendant_email,
        template: "summon_issued",
        data: {
