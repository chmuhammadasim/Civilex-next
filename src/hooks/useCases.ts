...
export function useCases() {
  ...
  const createCase = async (caseData: {
    ...
    try {
      ...
      // Create the case (case_number will be auto-generated by DB trigger)
    ...
    try {
      ...
      if (allowInstallments && installmentCount > 1) {
        ...
        if (paymentError) {
          ...
          return { error: "Failed to create payment records" };
        }

        if (firstPayment) {
          for (let i = 2; i <= installmentCount; i++) {
            const { error: installmentError } = await supabase.from("payments").insert({
              case_id: caseId,
              payer_id: actualPayerId,
              receiver_id: user.id,
            ...
            if (installmentError) {
              console.error(`Error creating installment ${i}:`, installmentError);
            }
          }
        }
      } else {
        const { data: singlePayment, error: paymentError } = await supabase.from("payments").insert({
          case_id: caseId,
          payer_id: actualPayerId,
          receiver_id: user.id,
          amount: feeAmount,
